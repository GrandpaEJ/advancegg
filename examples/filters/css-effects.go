...
func main() {
	...
	dc.SavePNG("images/filters/css-original.png")
...
func demonstrateCSSFilters(dc *advancegg.Context) {
	...
	for _, f := range filters {
		...
		newDC.SavePNG(fmt.Sprintf("images/filters/css-%s.png", f.name))
	...
	for _, preset := range presets {
		...
		newDC := advancegg.NewContextForRGBA(filtered.(*image.RGBA))
		newDC.SavePNG(fmt.Sprintf("images/filters/css-%s.png", preset.name))
	...
	customDC.SavePNG("images/filters/css-custom-chain.png")
...
func demonstratePatterns() {
	...
	for _, p := range patterns {
		...
		dc.SavePNG(fmt.Sprintf("images/filters/pattern-%s.png", p.name))
...
func createPatternComposite() {
	...
	dc.SavePNG("images/filters/pattern-gallery.png")
